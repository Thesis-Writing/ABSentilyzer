'''
  This module is intended for the ensemble algorithm functions to ensemble the results
  of SVM and MNB after the aspect term polarity classification, the algorithm was based on
  the study conducted by Korovkinas et. al. (2017) but was coded from scratch for implementation
'''

# Author            : Afrahly Afable
# Calling Sequence  : combine_final_prediction_to_array(similar_predictions, ensemble_predictions, ensemble_prob)
#                     ensemble_algorithm(different_predictions)
#                     ensemble(svm_predictions,mnb_predictions,svm_score,mnb_probabilities)
...
#                       - similar_predictions      : LIST
#                       - ensemble_predictions     : LIST
#                       - ensemble_prob            : LIST
#                       - different_predictions    : LIST
#                       - svm_predictions          : LIST
#                       - mnb_predictions          : LIST
#                       - svm_score                : LIST
#                       - mnb_probabilities        : LIST
#                     Output Variable/s:
#                       - final_prediction_array   : ARRAY
#                       - final_prob_array         : ARRAY
#                       - combined_pred_dict_list  : LIST
#                       - combined_prob_dict_list  : LIST
#                       - final_pred_array         : ARRAY
#                       - final_prob_array         : ARRAY

import numpy as np
import math

def combine_final_prediction_to_array(similar_predictions, ensemble_predictions, ensemble_prob):
  similar_prediction = {}
  final_prediction = {}
...
def ensemble(svm_predictions,mnb_predictions,svm_score,mnb_probabilities):
  ...
  if len(svm_pred_list) == len(mnb_pred_list):
    ...
    ensemble_predictions,ensemble_prob = ensemble_algorithm(diff_results_dict_list)
    final_pred_array,final_prob_array = combine_final_prediction_to_array(similar_results_dict_list, ensemble_predictions,ensemble_prob)
